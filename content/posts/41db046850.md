+++
title = """AWS FireLensを使ってみる"""
date = 2024-12-05T01:41:14.000Z
expiryDate = 2024-12-05T01:41:14.000Z
tags = ["vtj","devops"]
+++
ECSからCloudWatch Logsにログを流すのは難しくないですが、コスト的にLogsでログの長期保存はしたくありません。そこで過去のログをS3に退避して、Logsでは比較的短い保存期間を設定することがよくあります。

CloudWatch LogsにはS3にログをエクスポートする機能がるのですが、なぜか手動実行にしか対応していません。自動的にエクスポートするにはLambdaを定期実行するか、EventBridge Schedulerで定期的にエクスポートすることになります。
